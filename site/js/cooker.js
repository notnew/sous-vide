...
var graph = function (history, i) {
  ...
  var makeYScale = function () {
    if (valueName === "setting") {
      return d3.scale.linear().range([1, 0]);
    }
    ...
    else {     // target or temperature; use the same scale for both
      var min = d3.min(history, function (sample) {
        return Math.min(sample.temperature, sample.target); });
      var max = d3.max(history, function (sample) {
  ...
  var timescale = makeTimeScale(history);
  var scale = makeYScale();
